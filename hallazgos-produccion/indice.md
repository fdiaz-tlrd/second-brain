...
| HP-012 | Validación de `idSolicitud` responde 404 (reusado, colisiona con "Validador no existe") en vez de 431 | media-alta | `tld-validador-api` / `validar` | confirmado (código + Newman) | [11-validador-idSolicitud-usa-404-en-vez-de-431.md](11-validador-idSolicitud-usa-404-en-vez-de-431.md) |
| HP-013 | `idSolicitud` sin validación de charset → pasa y revienta en 509 (debe ser 431); incl. solo-guiones | media-alta | `tld-validador-api` / `validar` | confirmado — se corrige en dev | [12-validador-idSolicitud-sin-validacion-charset.md](12-validador-idSolicitud-sin-validacion-charset.md) |
| HP-014 | Elemento `null` en `solicitudes[]` crashea la validación → 999 (debe ser 431) | alta | `tld-validador-api` / `validar` | confirmado — se corrige en dev | [13-validador-solicitud-null-crashea-999.md](13-validador-solicitud-null-crashea-999.md) |
| HP-015 | Canal emisor sin plan de suscripción no se rechaza → 509 (debe ser 403) | media-alta | `tld-api-cuenta-nombre` / `cuenta-nombre` | confirmado — se corrige en dev | [14-vcn-sin-plan-suscripcion-enmascara-509.md](14-vcn-sin-plan-suscripcion-enmascara-509.md) |
| HP-016 | Canal emisor mal configurado en BD → 405 descifrado (debe ser 500) | media-alta | `tld-validador-api` / `validar` | confirmado — se corrige en dev | [15-vcn-canal-mal-configurado-responde-405.md](15-vcn-canal-mal-configurado-responde-405.md) |
...
| HP-018 | Método fuera de `CFG_METODOS_LIMITES_JSON` → 509 (debe ser 418); regla operativa métodos por canal | media-alta | `tld-validador-api` / `validar` | confirmado — se corrige en dev | [17-metodo-fuera-cfg-enmascara-509.md](17-metodo-fuera-cfg-enmascara-509.md) |
